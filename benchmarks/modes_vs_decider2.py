"""Flagship batch throughput and score() latency: decider (every mode) against decider2 fused.

decider2's compiled path needs its `_nashim` extension built for this interpreter:

    PYTHONPATH=<decider2 src with _nashim built> uv run python benchmarks/modes_vs_decider2.py
"""
import gc
import time
...
from decider2.examples.flagship import pipeline as flagship2
...
flagship2.precompile()
engines = {"decider2 fused": (lambda df: flagship2.apply(df, mode="fused"), flagship2.score)}
