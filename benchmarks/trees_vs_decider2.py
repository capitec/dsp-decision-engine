"""Tree walk throughput and score() latency: decider's TreeConfig (every mode) against decider2's tree_module.
...
a `channel` column (starts_with, byte spans in the kernel) in front of it. decider2 writes String columns with
`decode()` after the run, which is included in its batch time; its `score()`
returns no String column.

decider2's compiled path needs its `_nashim` extension built for this interpreter:

    PYTHONPATH=<decider2 src with _nashim built> uv run python benchmarks/trees_vs_decider2.py
"""
import gc
import time
...
from decider2.trees import Tree as Tree2
...
def compare(doc: dict) -> None:
    built = tree_module(Tree2.model_validate(doc), name="campaign", feature_types={f: int for f in INTS})
    pipeline2 = flow2(built.module)
    pipeline2.precompile()
    engines = {"decider2 fused": (lambda df: built.decode(pipeline2.apply(df, mode="fused")), pipeline2.score)}
    ...
    expected = engines["decider2 fused"][0](frame.head(20000)).select("label", "score", "band")
