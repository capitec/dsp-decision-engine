...
def test_running_never_calls_a_step_on_values_the_caller_did_not_supply():
    assert flow(inv).run(pl.DataFrame({"x": [5.0, 3.0]}))["inv"].to_list() == [0.25, 0.5]
...
def test_a_step_may_narrow_a_value_it_also_reads():
    out = flow(product_ceiling, cap_by_income).run(WATERFALL_FRAME)
    assert out["term_cap"].to_list() == [60.0, 48.0]


def test_the_version_chain_matches_the_answer():
    out = flow(product_ceiling, cap_by_income).emit("term_cap@*").run(WATERFALL_FRAME)
    assert out["term_cap@product_ceiling"].to_list() == [60.0, 60.0]
    assert out["term_cap@cap_by_income"].to_list() == [60.0, 48.0]
    assert out["term_cap"].to_list() == [60.0, 48.0]


def test_a_single_version_can_be_emitted_by_its_producer():
    out = flow(product_ceiling, cap_by_income).emit("term_cap@product_ceiling").run(WATERFALL_FRAME)
    assert out["term_cap@product_ceiling"].to_list() == [60.0, 60.0]


def test_a_self_read_waterfall_seeded_from_the_frame_overwrites_the_input_column():
    out = flow(cap_by_income).run(pl.DataFrame({"term_cap": [60.0, 60.0], "min_net_salary": [9200.0, 4100.0]}))
...
def test_a_name_resembling_its_own_output_is_not_an_error():
    assert flow(term_cap_a).run(pl.DataFrame({"term_cap": [60.0]}))["term_cap_a"].to_list() == [59.0]
...
def test_an_int_step_returns_an_integer_column():
    ...
    out = flow(cents).run(pl.DataFrame({"rands": pl.Series([12, 34], dtype=pl.Int64)}))
...
def test_int64_above_2_to_the_53_is_not_degraded():
    ...
    out = flow(passthrough).run(pl.DataFrame({"n": pl.Series([big], dtype=pl.Int64)}))
...
def test_a_bool_step_returns_a_boolean_column():
    ...
    out = flow(is_thin).run(pl.DataFrame({"score": [550.0, 700.0]}))
    assert out["is_thin"].dtype == pl.Boolean
    assert out["is_thin"].to_list() == [True, False]


def test_a_string_input_reaches_the_step_as_a_string():
...
def test_a_misspelled_param_is_rejected():
    ...
    with pytest.raises(ParamsError, match="unknown param 'capp'; did you mean 'cap'"):
        flow(capper).run(pl.DataFrame({"term_cap": [60.0]}), params={"capper": {"capp": 10.0}})


def test_a_frame_column_shadowing_a_step_output_is_an_error():
...
def test_a_string_literal_declared_as_a_param_compares_correctly():
    assert flow(sector_rate).run(SECTOR_FRAME)["sector_rate"].to_list() == [0.9, 1.0, 0.9, 1.0]


def test_changing_a_string_literal_is_a_value_change():
    out = flow(sector_rate).run(SECTOR_FRAME, params={"sector_rate": {"private": "government"}})
    assert out["sector_rate"].to_list() == [1.0, 1.0, 1.0, 0.9]


def test_a_literal_absent_from_the_data_never_matches():
    out = flow(sector_rate).run(SECTOR_FRAME, params={"sector_rate": {"private": "martian"}})
