...
def test_a_named_flow_is_tuned_through_its_path():
    exe = Engine().bind(band)
...
def test_score_is_namespaced_the_same_way_as_run():
    exe = Engine().bind(band)
    assert exe.score({"term_cap": 60.0}, params={"band": {"net_of_floor": {"floor": 20.0}}})["plus_headroom"] == 43.0


def test_a_shared_param_is_read_from_the_shared_entry():
    assert band.run(FRAME, params={"shared": {"base": 1.0}})["plus_headroom"].to_list() == [54.0, 54.0]
...
def test_an_unknown_params_namespace_is_a_hard_error_with_a_suggestion():
    with pytest.raises(ParamsError, match="no step with params at 'kap'. Did you mean 'cap'?"):
        flow(cap).run(FRAME, params={"kap": {"cap": 12.0}})


def test_an_unknown_nested_namespace_is_a_hard_error_with_a_suggestion():
    with pytest.raises(ParamsError, match="'band/net_of_flor'. Did you mean 'band/net_of_floor'?"):
        band.run(FRAME, params={"band": {"net_of_flor": {"floor": 1.0}}})


def test_an_unknown_shared_key_is_a_hard_error_with_a_suggestion():
    with pytest.raises(ParamsError, match="shared param 'bse'. Did you mean 'base'?"):
        band.run(FRAME, params={"shared": {"bse": 1.0}})


def test_the_namespace_is_checked_in_lazy_mode_too():
    with pytest.raises(ParamsError, match="'kap'"):
        Engine(params_validation="lazy").bind(flow(cap)).run(FRAME, params={"kap": {"cap": 12.0}})
