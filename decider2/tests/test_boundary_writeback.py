"""Scratch tests for decider2.boundary.writeback — doc 05 §3.1's dtype-grouped
2D arrays, and doc 03 §7's additive output frame.
"""
import numpy as np
import polars as pl
import pytest

from decider2.boundary.writeback import (
    DtypeGroup,
    KernelOutputs,
    Layout,
...
def test_row_to_dict_merges_all_groups_for_one_record():
    ...
    assert isinstance(result["approved"], bool)  # tolist() gives Python bool, not np.bool_
