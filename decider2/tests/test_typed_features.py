...
def test_the_string_slot_carries_polars_bytes_zero_copy_as_spans():
    ...
    (`boundary.extract`), so a kernel can read the actual string
    ...
    # Stage 3: the whole frame crosses once, and the STR slot of the gather
    # IS the span table — there is no per-column extraction any more.
    extracted = extract_frame(pl.DataFrame({"s": strings}), [Input("s", bytes)])
    spans = extracted.columns["s"].values
    assert spans.shape == (5, 2) and spans.dtype == np.int64
    assert spans[:, 1].tolist() == [len(s) for s in strings]
    registry = {"s": spans, "k": np.array([1, 2, 3, 4, 5], dtype=np.int64)}
    ...
    # `ExtractedFrame.kernel_frame` owned polars' buffers; the spans above
    # were only read while it lived.
    del spans, extracted
