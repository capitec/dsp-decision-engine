"""decider2.boundary.extract — the whole frame crosses ONCE through Arrow
(docs/BOUNDARY-REWORK.md §1, Stage 3).

Every value is compared against what Python sees through polars' own
`to_list()`/`to_physical()`; every frame shape the design names (fresh,
sliced, `pl.concat` multi-chunk, a nullable numeric with the null on row 0
and on the last row) must extract identically.
"""
import ctypes
import threading
...
from decider2.boundary.extract import NeedsKernelSplit, _local, _schema_plan
...
# --- every row of §1.2's table round-trips ------------------------------------
...
def test_floats_stay_floats_ints_stay_ints_bools_stay_bools():
    ...
    for ec in ex.columns.values():
        assert ec.values.flags.writeable is False and ec.values.flags.c_contiguous  # doc 05 §1.4: read-only
...
def test_a_str_input_arrives_as_dictionary_codes_with_the_exported_dictionary():
    """A String column declared `str` is cast to Categorical in the frame
    tier (the code convention hand-written steps keep until Stage 7) and
    the categories are the EXPORTED dictionary, not `cat.get_categories()`:
    polars' physical codes are process-global, its Arrow dictionary is per
    batch (measured on 1.41.2 and 1.44.2)."""
...
def test_missing_as_fills_in_the_gather_and_reports_what_it_filled():
    ...
    assert f.values.tolist() == [10.0, 0.0, 30.0] and f.fill.filled_count == 1 and f.fill.filled_mask.tolist() == [False, True, False]
    assert i.values.tolist() == [-7, 2, -7] and i.fill.filled_count == 2
    assert b.values.tolist() == [True, True, True] and b.fill.filled_count == 2
    assert f.validity is None and f.fill.reason is FillReason.MISSING          # a filled column needs no mask


def test_not_applicable_as_fills_identically_but_tags_a_different_reason():
    frame = pl.DataFrame({"x": [None, 500.0]})
    ex = extract_frame(frame, [Input("x", float, NullPolicy.NOT_APPLICABLE_AS, fill=0.0)])
    assert ex.columns["x"].values.tolist() == [0.0, 500.0]
    assert ex.columns["x"].fill.reason is FillReason.NOT_APPLICABLE


def test_a_clean_missing_as_column_reports_nothing_filled():
    ...
    assert ex.columns["x"].fill.filled_count == 0 and ex.columns["x"].fill.filled_mask is None
...
def test_optional_keeps_a_validity_mask_and_never_fills():
    ...
    col = ex.columns["x"]
    assert col.validity.tolist() == [True, False] and col.fill is None
...
def test_the_garbage_slot_under_a_null_is_never_read():
    ...
    zero'. Build a column with a nonzero value physically present under a
    null slot and confirm the gather wrote the fill, not the garbage."""
...
def test_fresh_sliced_and_multi_chunk_frames_extract_identically():
    ...
    # caller's frame IN PLACE (§9) — both halves pinned here.
...
def test_a_zero_row_frame_extracts_zero_length_columns_of_the_right_dtype():
    ...
    assert ex.kernel_frame.height == 0 and ex.routing.routed_count == 0
...
# --- routing, absent columns, the whole-frame contract ------------------------

def test_extract_frame_removes_routed_rows_from_the_kernel_frame():
    frame = pl.DataFrame({"instalment": [100.0, None, 300.0], "id": [1, 2, 3]})
    inputs = [Input("instalment", float), Input("id", int)]
    result = extract_frame(frame, inputs)
    assert result.routing.routed_count == 1 and result.routing.decision is Decision.REFER
    assert result.kernel_frame.height == 2
    assert result.columns["instalment"].values.tolist() == [100.0, 300.0]
    assert result.columns["id"].values.tolist() == [1, 3]


def test_extract_frame_raise_for_fails_before_any_export():
    frame = pl.DataFrame({"application_id": [1, None]})
    with pytest.raises(ValueError, match="application_id"):
        extract_frame(frame, [Input("application_id", int)], policy=MissingInputPolicy(raise_for=("application_id",)))


def test_a_required_column_entirely_absent_routes_every_row_and_synthesizes_an_empty_column():
    frame = pl.DataFrame({"a": [1.0]})
    result = extract_frame(frame, [Input("a", float), Input("missing_col", float)])
    assert set(result.columns) == {"a", "missing_col"}
    assert result.routing.routed_count == 1 and result.kernel_frame.height == 0
    assert result.columns["missing_col"].values.tolist() == []


def test_absent_columns_are_synthesized_per_tier_in_the_kinds_own_dtype():
    frame = pl.DataFrame({"a": [1.0, 2.0, 3.0]})
    ex = extract_frame(frame, [
        Input("a", float),
        Input("f", float, NullPolicy.MISSING_AS, fill=7.0),
        Input("i", int, NullPolicy.NOT_APPLICABLE_AS, fill=-1),
        Input("o", int | None, NullPolicy.OPTIONAL),
        Input("s", str, NullPolicy.OPTIONAL),
        Input("r", bytes, NullPolicy.OPTIONAL),
    ])
    assert ex.columns["f"].values.tolist() == [7.0] * 3 and ex.columns["f"].fill.filled_count == 3
    assert ex.columns["i"].values.tolist() == [-1] * 3 and ex.columns["i"].fill.reason is FillReason.NOT_APPLICABLE
