/* shim.c -- decider2's C shim between numba and vendored nanoarrow 0.9.0.
 *
 * docs/BOUNDARY-REWORK.md §4.1. Every value this file hands out is a
 * nanoarrow field or accessor result: `buffer_views`, `offset`,
 * `storage_type`, `ArrowArrayViewGetStringUnsafe`, `ArrowBitGet`. The
 * per-row loads are `data[offset + i]`, which is what nanoarrow's own
 * `ArrowArrayViewGet*Unsafe` do. There is NO Arrow-layout knowledge here:
 * nothing decodes a Utf8View element, an offsets buffer or a bitmap by hand.
 *
 * It is plain C99 (no Python) so the numba kernel can call `sm_gather_row`
 * and `sm_get_string` through raw addresses passed as ARGUMENTS
 * (EXPERIMENTS.md §W: a pointer argument disk-caches; a captured symbol does
 * not). shim_module.c wraps it as the CPython extension
 * `decider2._arrow._nashim` and exports each `sm_*` address as an int.
 *
 * Union of experimentation/nanoarrow-accessors/c/nashim.c (import, view
 * accessors, get_string) and experimentation/boundary-rework-design/c/
 * rowshim.c (resolve + gather), with `sm_get_string_checked` dropped (the
 * owner's decision: the project owns zero lines of layout) and the kinds
 * renumbered to decider2.types.FeatureKind.
 */
#include <math.h>
#include <stdint.h>
#include <string.h>
...
/* decider2.types.FeatureKind -- program data, never renumbered. */
 ...
 * when the validity bit is clear (MISSING_AS / NOT_APPLICABLE_AS ride here,
 * BOUNDARY-REWORK.md §1.5); the defaults are NaN / 0 / -1 (CODE).       */
struct SmColDesc {
 ...
 * the request path never validates -- BOUNDARY-REWORK.md §0.1).          */
 ...
 * Which Arrow storage types land in which kind is BOUNDARY-REWORK.md §1.2's
 * table. Returns 0, or -1 when the child's Arrow type does not match the
...
int sm_resolve_col(const struct ArrowArrayView* v, int32_t kind, int32_t slot,
  ...
  switch (kind) {
    ...
    case SM_KIND_STR:
      if (encoded) return -1;  /* Stage 6: strings through the dictionary */
