...
def _build_path_fn(
    ...
    if n_computed == 0:
        # `inline="always"`: the driver's per-row kernel inlines this body
        # (and, through it, `walk_tree`) at numba-IR level, so the six row
        # arrays never cross a real call boundary per row — see
        # `interpreter.walk_tree`'s docstring for the measured reason.
        # Still `cache=True`: `interpreted`/`stepped` call this standalone
        # from Python, and THAT specialisation (walk_tree inlined into it)
        # is the one the disk cache holds.
