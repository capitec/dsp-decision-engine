...
from engine.util import get_name
...
def build_score_criteria(variable: pd.Series, criteria_type: ScoreCriteriaTypes = None, variable_name: typing.Optional[str]=None, **kwargs) -> ScoreCriteria:
    if isinstance(variable, str):
        variable_name = variable
        variable_value = None
        assert criteria_type is not None, "Criteria type must be specified when score criteria is not created with a pd.Series variable type"
    else:
    ...
    if criteria_type == "numerical":
        ...
        raise ValueError(f"Invalid Criteria type {criteria_type} expecting either \"numerical\" or \"categorical\"")
