...
from engine.components.scorecard.v2.common import DiscreteScorePattern, DefaultScorePattern, RangeScorePattern
from engine.components.scorecard.v2.criteria_categorical import ScoreCriteriaCategorical
from engine.components.scorecard.v2.criteria_numerical import ScoreCriteriaNumerical, Bounds
from engine.components.scorecard.v2.scorecard import ScoreCardModel
...
class DiscreteScorePatternViewModel(traitlets.HasTraits):
    ...
    @staticmethod
    def from_pydantic_model(model: 'DiscreteScorePattern[typing.Optional[str]]'):
    ...
    def get_widget(self, should_display=True, on_delete_callback=None):
        ...
        delete_row = widgets.Button(
            ...
            button_style='danger',
            icon='trash',
            layout=widgets.Layout(right='5px', top = '5px', width='32px', height='28px', ),
        )
        ...
        categorical_row_config_grid = widgets.GridspecLayout(1, 6, layout=widgets.Layout(width='100%', grid_gap='5px'))
...
class DiscreteFloatScorePatternViewModel(traitlets.HasTraits):
    ...
    @staticmethod
    def from_pydantic_model(model: 'DiscreteScorePattern[float]'):
    ...
    def convert_float(value: str):
        ...
        if lower_val in ['none', 'null']: return None
        if lower_val == 'inf': return float('inf')
        if lower_val == '-inf': return -float('inf')
        return float(value)
    ...
    def get_widget(self, should_display=True, on_delete_callback=None):
        ...
        categorical_row_config_grid[0,1]=score_idx
        categorical_row_config_grid[0,2:4]=score_description
        categorical_row_config_grid[0,4:]= widgets.HBox([widgets.Label(value="Values: ", layout=widgets.Layout(width='6em')), values], layout=widgets.Layout(display='grid', grid_template_columns='auto 1fr'))
...
class DefaultScorePatternViewModel(traitlets.HasTraits):
    ...
    @staticmethod
    def from_pydantic_model(model: 'typing.Optional[DefaultScorePattern]') -> 'DefaultScorePatternViewModel':
        if model is None: return DefaultScorePatternViewModel(is_active=False)
    ...
    def get_widget(self, should_display=True):        
        ...
        categorical_row_config = widgets.GridspecLayout(1, 12, layout=widgets.Layout(width='100%', grid_gap='5px'))
...
class RangeScorePatternViewModel(traitlets.HasTraits):
    ...
    def get_widget(self, should_display=True, on_delete_callback=None):        
        ...
        upper_inf.observe(lambda change: on_set_bounds(upper_bound, change), names='value')
        on_set_bounds(upper_bound, {'new':upper_inf.value})
        lower_inf.observe(lambda change: on_set_bounds(lower_bound, change), names='value')
        on_set_bounds(lower_bound, {'new':lower_inf.value})

        categorical_row_config_grid = widgets.GridspecLayout(1, 12, layout=widgets.Layout(width='100%', grid_gap='5px'))
        ...
        categorical_row_config_grid[0,8:10]=widgets.Box(
            ...
            layout=widgets.Layout(width='auto', display='grid', grid_template_columns='2.5em auto auto')
        )
        categorical_row_config_grid[0,10:]=widgets.Box(
            [widgets.Label("Max: ", layout=widgets.Layout(width='4em')), upper_inf, upper_bound], 
            layout=widgets.Layout(width='auto', display='grid', grid_template_columns='2.5em auto auto')
        )
...
class ScoreCriteriaCategoricalViewModel(traitlets.HasTraits):
    ...
    def from_pydantic_model(model: 'ScoreCriteriaCategorical'):
        return ScoreCriteriaCategoricalViewModel(
            ...
            discrete_scores = [DiscreteScorePatternViewModel.from_pydantic_model(v) for v in model.discrete_scores],
    ...
    def get_widget(self, should_display=True, on_variable_update=None): 
        ...
        default_behaviour = widgets.Dropdown(
            value='matches',
            options=["regex","regex_end","regex_start","regex_partial","matches","matches_end","matches_start","matches_partial"],
        ...
        add_btn = widgets.Button(
            icon='plus',
            layout=widgets.Layout(width='45px', height='28px', ),
        )

        def add_dv(evnt):
            new_item = DiscreteScorePatternViewModel()
            new_view = new_item.get_widget(should_display=False, on_delete_callback=delete_discrete_var)
        ...
        discrete_box = widgets.VBox(
            [v.get_widget(should_display=False, on_delete_callback=delete_discrete_var) for v in self.discrete_scores],
            layout = widgets.Layout(width='100%')
        )
...
class ScoreCriteriaNumericalViewModel(traitlets.HasTraits):
    ...
    def from_pydantic_model(model: ScoreCriteriaNumerical):
        return ScoreCriteriaNumericalViewModel(
            ...
            discrete_scores = [DiscreteFloatScorePatternViewModel.from_pydantic_model(v) for v in model.discrete_scores],
            range_scores = [RangeScorePatternViewModel.from_pydantic_model(v) for v in model.range_scores],
            other_score = model.other_score
        )
    ...
    def get_widget(self, should_display=True, on_variable_update=None): 
        ...
        add_btn = widgets.Button(
            ...
            icon='plus',
            layout=widgets.Layout(width='12em', height='28px', ),
        )
        ...
        def add_rv(evnt):
            ...
            new_view = new_item.get_widget(should_display=False, on_delete_callback=delete_range_var)
            # Have to make new list for traitlet to emit event
        ...
        header = widgets.HBox(
            [variable_name, widgets.Label("Included Bounds: ",layout=widgets.Layout(width='12em')), upper_bound, lower_bound, add_range_btn, add_btn],
            layout=widgets.Layout(
                width='100%'
            )
        )
        ...
        range_box = widgets.VBox(
            [v.get_widget(should_display=False, on_delete_callback=delete_range_var) for v in self.range_scores],
            layout = widgets.Layout(width='100%')
        )

        view = widgets.VBox(
            [header, range_box, discrete_box, self.other_score.get_widget(should_display=False)],
            layout = widgets.Layout(width='100%')
        )
...
class ScoreCardViewModel(traitlets.HasTraits):
    ...
    def from_pydantic_model(cls, model: ScoreCardModel):
        variable_params = [
            ...
            if isinstance(vp, ScoreCriteriaCategorical) else
                ScoreCriteriaNumericalViewModel.from_pydantic_model(vp)
    ...
    def to_pydantic_model(self) -> ScoreCardModel:
        variable_params = [
            vpm.to_pydantic_model()
            for vpm in self.variable_params
        ]
    ...
    def get_widget(self, should_display=True): 
        ...
        add_categorical_button = widgets.Button(
            description='Add Categorical',
            layout=widgets.Layout(width='auto')
        )
        add_numerical_button = widgets.Button(
            description='Add Numerical',
            layout=widgets.Layout(width='auto')
        )
        ...
        def delete_variable_param(id_param):
            ...
            for el, vp in zip(variable_parameter_container.children, self.variable_params):
                if id(vp) == id_param: continue
                new_vp.append(vp)
        ...
        def get_vp_widget(vp):
            ...
            child_widget = vp.get_widget(
                ...
                on_variable_update=lambda change: update_accordian_name(accordion, change['new'], variable_type)
            )
        ...
        view = widgets.VBox([header_grid, variable_parameter_container], layout=widgets.Layout(width='100%'))
        if should_display:
            display(view)
        return view
