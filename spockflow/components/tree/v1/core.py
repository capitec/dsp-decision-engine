...
class Tree(VariableNode):
    ...
    def _generate_runtime_nodes(self, config: "typing.Dict[str, typing.Any]", compiled_node: "CompiledNumpyTree") -> "typing.List[node.Node]":
        from hamilton import node
        return [
            node.Node.from_fn(
                compiled_node._flattened_tree.conditions[exec_cond], 
                name=exec_cond
            )
    ...
    def condition(self, 
        ...
        child_tree: ChildTree=None, **kwargs
    ...
    def visualize(self, get_value_name=None, get_condition_name=None):
        ...
        if get_condition_name is None:
            from spockflow._util import get_name
            get_condition_name = lambda x: x if isinstance(x, str) else get_name(x, None)
        ...
        while to_search:
            ...
            for node in curr.nodes:
                ...
                if hasattr(node.value, 'nodes'):
                    ...
                    dot.node(node_value_name, node_value_name, style='filled',fillcolor='#ADDFFF',shape='rectangle')
            ...
            if curr.default_value is not None:
                ...
                dot.node(default_name, default_name, style='filled',fillcolor='#ADDFFF',shape='rectangle')
