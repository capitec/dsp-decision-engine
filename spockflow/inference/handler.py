...
if typing.TYPE_CHECKING:
    ...
    from spockflow.inference.model_loader import ModelLoader, TConfigLoaderMap, TModuleList, TModuleListMap
...
class ServingHandler:
    ...
    def __init__(self, **kwargs):
        ...
        self.model_loader_cls = kwargs.get("model_loader_cls", lazy_load_default_model_loader)
        self.model_cache_cls = kwargs.get("model_cache_cls", lazy_load_default_model_cache)
        self.model_config_cls = kwargs.get("model_config_cls", lazy_load_default_model_config)
        ...
        for method_name in filter(lambda x: x.endswith("_fn"), dir(self)):
            if not callable(getattr(self,method_name)): continue # Only override functions
    ...
    def model_fn(self, model_dir: str) -> typing.Union["TModuleList","TModuleListMap","ModuleType"]:
    ...
    def _schedule_refresh_latest_model(self, schedule_interval: int):
        ...
        self._model_cache_refresh_tasks.append(
            loop.create_task(refresh_latest_model())
        )
    ...
    def setup_model_cache_fn(self):
        ...
        if (
            (settings.server_pre_init_default_model is None and None in self.model_cache.model_loader) or
            (settings.server_pre_init_default_model)
        ):
    ...
    def input_fn(self, input_data: bytes, content_type: str):
        if content_type not in self.decoders:
            ...
            raise UnsupportedEncoding.from_content_type(content_type, list(self.decoders.keys()))
