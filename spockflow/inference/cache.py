...
class LatestVersionTTLCache():
    ...
    def set(self, value, version):
        ...
        if old_item == self._Unset: return None
...
class ModelCache(LRUCache, typing.Generic[T]):
    ...
    def __getitem__(self, key) -> T:
        if key is None: return self.latest_item_cache.get()
...
class ModelCacheManager():
    def __init__(self, model_loader: "ModelLoader", max_size=None, latest_ttl=None) -> None:
        if max_size is None:
    ...
    def get(self, model_name: str = None, version: str = None):
        if model_name not in self.model_loader:
            if model_name is None:
                raise ValueError(f"No default model specified please specify model to run with {get_settings().server_model_name_header}")
