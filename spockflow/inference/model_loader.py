...
class ModelLoader():
    ...
    def load_model(
        ...
        try:
            config_loader = self.config_loaders.get(model_name, self.config_loaders["__default__"])
        except KeyError as e:
            logger.exception(e)
            raise ValueError(f"Could not infer the correct config loader to use for model: \"{model_name}\"") from e
        ...
        try:
            ...
            logger.exception(e)
            raise ValueError(f"Could not load config for \"{model_name}\" \"{model_version}\"") from e
