...
def load_entrypoint_modules(module_list: typing.List[str]) -> "TModuleList":
    ...
    if len(module_list) <= 0:
        raise ValueError("Module list must contain at least one module to be loaded by spock")
...
def load_all_entrypoints(entrypoint_configuration: typing.Dict[str, typing.List[str]]) -> "TModuleListMap":
...
def cast_result_of_model_fn(modules: typing.Union["TModuleList","TModuleListMap","ModuleType"]) -> "TModuleListMap":
    """ Allow user to return multiple types from the model fn and cast it to the form expected """
    if isinstance(modules, dict): return modules
