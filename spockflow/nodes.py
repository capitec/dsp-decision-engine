...
class VariableNodeFunctionParameters:
    ...
    kwarg_input_generator: typing.Union[typing.Callable[[object,typing.Callable], typing.Dict[str,typing.Any]],str] = None
    
    def get_node_name(self, parent_name, function_name):
        node_name_template = self.node_name_template
        if node_name_template is None:
            node_name_template = "{function_name}" if self.is_namespaced else "{parent_name}"
...
assert base.NodeCreator.get_lifecycle_name() == 'generate', "The version of Hamilton you are using is incompatible with this generation"
class VariableNode(BaseModel):
    ...
    def _generate_nodes(self, name: str, config: "typing.Dict[str, typing.Any]", include_runtime_nodes: bool=False) -> "typing.List[node.Node]":
        ...
        node_functions = inspect.getmembers(compiled_variable_node, predicate=self._does_define_node)
        namespaced_nodes = []
