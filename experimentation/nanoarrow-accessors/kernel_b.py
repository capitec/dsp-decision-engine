"""Approach B: the STR node asks nanoarrow for each row's bytes through
`sm_get_string` (a function pointer ARGUMENT), then matches the bytes with
the incumbent's byte-matcher. Per-row Arrow-layout knowledge in this file: none.
The tree-walk (LEAF/CMP/IS_TRUE) is copied verbatim from the incumbent."""
...
def run_tree_b(float_cols, nano_views: list[NanoView], tree, patterns, out=None, checked=False,
               pat=None):
